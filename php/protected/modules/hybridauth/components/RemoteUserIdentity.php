...
class RemoteUserIdentity extends CBaseUserIdentity {
	...
	public function authenticate() {
		...
		if ($adapter->isUserConnected()) {
			...
			$user = User::findUserRemote($this->loginProvider, $this->loginProviderIdentity);
                        echo '<pre>';
                        var_dump($this);
                        echo '</pre>';die();
                        if($user == null ){

                            $datos = $adapter->adapter->user->profile;

                            $parametros = array('userId'=>1,'loginProvider'=>$this->loginProvider,'loginProviderIdentity'=>$this->loginProviderIdentity,'username'=>$datos->email,"email"=>$datos->email);

                            $user_ = new User;

                            $user_->attributes=$parametros;
